...
csv_output = "./results/evaluation_results.csv"
...
transform10 = transforms.Compose(
    [transforms.ToTensor(), transforms.Normalize(mean=mean10, std=std10)]
)
transform100 = transforms.Compose(
    [transforms.ToTensor(), transforms.Normalize(mean=mean100, std=std100)]
)
...
def evaluate(model_name, dataset_loader, cifar_type="10"):
    ...
    model = torch.hub.load(
        "chenyaofo/pytorch-cifar-models", model_name, pretrained=False, trust_repo=True
    )
    # load saved weights only
    state = torch.load(os.path.join(model_dir, f"{model_name}.pth"), weights_only=True)
    ...
    # Save result to the results list
    results.append(
        {"model_name": model_name, "dataset": f"CIFAR-{cifar_type}", "accuracy": acc}
    )
...
with open(csv_output, "w", newline="") as csvfile:
    fieldnames = ["model_name", "dataset", "accuracy"]
