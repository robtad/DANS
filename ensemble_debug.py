...
class DistillationModel(nn.Module):
    ...
    def get_soft_targets(self, x):
        with torch.no_grad():
            teacher_logits = self.teacher(x)
            return F.softmax(teacher_logits / self.temperature, dim=1)

def train_distilled_model(teacher_model, dataset_loader, num_classes, epochs=10):
    """Train student model with defensive distillation"""
    # Create student model (same architecture)
    student_model = torch.hub.load(
        "chenyaofo/pytorch-cifar-models",
        teacher_model.__class__.__name__.lower(),
        pretrained=False,
        trust_repo=True,
    ).to(DEVICE)
    
    distill_model = DistillationModel(teacher_model, student_model, DISTILL_TEMP)
    optimizer = torch.optim.Adam(student_model.parameters(), lr=0.001)
    
    student_model.train()
    for epoch in range(epochs):
        for batch_idx, (images, labels) in enumerate(dataset_loader):
            if batch_idx > 20:  # Quick training for demonstration
                break
                
            images, labels = images.to(DEVICE), labels.to(DEVICE)
            
            # Get soft targets from teacher
            soft_targets = distill_model.get_soft_targets(images)
            ...
            distill_loss = F.kl_div(
                torch.log(student_probs), soft_targets, reduction='batchmean'
            )
            ...
            # Combined loss
            total_loss = DISTILL_ALPHA * distill_loss + (1 - DISTILL_ALPHA) * ce_loss
...
def main():
    ...
    for model_name in MODEL_NAMES:
        ...
        print("  → Training distilled model...", end="", flush=True)
        train_loader = torch.utils.data.DataLoader(
            balanced_subset, batch_size=BATCH_SIZE, shuffle=True, num_workers=4
        )
        distilled_model = train_distilled_model(model, train_loader, dataset_params['num_classes'])
