...
class FixedEnsembleAttack:
    ...
    def debug_individual_attacks(self, images, labels):
        ...
        with torch.no_grad():
            ...
            print(
                f"      Individual attacks - FGSM: {fgsm_acc:.1f}%, PGD: {pgd_acc:.1f}%, BIM: {bim_acc:.1f}%"
            )
...
def evaluate_ensemble_attacks(model, test_loader, model_name):
    ...
    for imgs, labels in test_loader:
        imgs, labels = imgs.to(DEVICE), labels.to(DEVICE)
        adv_imgs = ensemble_attack.weighted_ensemble_attack(
            imgs, labels, weights=[0.4, 0.3, 0.3]
        )
        with torch.no_grad():
            adv_preds = model(adv_imgs).argmax(1)
    ...
    print(
        f" acc: {wea_alt_acc:.2f}%  drop: {wea_alt_drop:.2f}%  time: {wea_alt_elapsed:.1f}s"
    )
...
def main():
    ...
    print(
        f"Balanced test set size: {len(balanced_test_ds)} images ({samples_per_class} per class)"
    )
    ...
    for _, row in df.iterrows():
        print(
            f"{row['model']:<25} {row['attack']:<8}: {row['clean_acc']:.1f}% → {row['adv_acc']:.1f}% (drop: {row['acc_drop']:.1f}%)"
        )
